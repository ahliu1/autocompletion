 ...
 * @author Amy Liu, topMatches() implementation
 */
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		// initiate dummy with given prefix and dummy weight value of 0 to compare
		...
		if (first == -1 || k == 0) {               // prefix not found or asking for 0 matches
		...
		// looping through all terms (halving each time to find correct string)
		for (int i = first; i <= last; i++){
			Term tee = myTerms[i];
			if (!tee.getWord().startsWith(prefix)){ 
				// skips loop if term doesn't start with prefix
				continue;
			}
			// checking if priority queue has room to add more terms
			if (pq.size() < k) {
				pq.add(tee);
			} else if (pq.peek().getWeight() < tee.getWeight()) { 
				// checking if current term has greater weight than head of priority queue
				// (aka lowest priority item, since priority queue is ordered in ascending order)
				pq.remove(); // if so; remove that head and replace with term
				pq.add(tee);
			}
		}

		// makes sure numResults doesn't exceed max number results requested/num result available in pq
		int numResults = Math.min(k, pq.size()); 
		LinkedList<Term> ret = new LinkedList<>();
		for (int i = 0; i < numResults; i++) {
			// add all items of Priority Queue to LinkedList
